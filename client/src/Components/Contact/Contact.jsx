...
export default function Contact() {
    ...
    function handleOnChange(e) {
        const objError = validate({ ...input, [e.target.name]: e.target.value }, e.target.name, error, setError);
    ...
    function errorExist(e) {
        ...
        if (!input.user_name.length || !input.user_email.length || !input.user_affair.length || !input.user_message.length) {
            setErrorsExist(true);
            setNotError(false);
        ...
        if (
            ...
            if (input.user_name && input.user_email && input.user_affair && input.user_message) {
                emailjs.sendForm(
                    ...
                    .then(res => res === undefined && setEnableErrors({
                        name: false,
                        email: false,
                        affair: false,
                        message: false,
        ...
        <div className={`container-fluid p-0 ${styles.container}`}>
            ...
            <div className={`d-flex mb-2 col-12 ${styles.container_Info}`}>
                ...
                <div className={`${styles.container_infoAndForm}`}>
                    ...
                    <form ref={form} onSubmit={(e) => handleSubmit(e)} className={`container-fluid d-flex justify-content-center col-12 p-0`} >
                        <div className={`row col-12`}>
                            <div className='mb-3'>
                                <input
                                    ...
                                    ms-0 
                                    ${styles.inputs}
                                    ${(enableErrors.name && cleanForm === false) ? (error.user_name ? 'is-invalid' : 'is-valid') : 'form-control'}`}
                                    placeholder='Nombre *'
                                    name='user_name'
                                ...
                                />
                                {
                                    error.user_name && <p className="invalid-feedback mb-0">{error.user_name}</p>
                                }
                            </div>
                            <div className='mb-3'>
                                <input
                                    ...
                                    ms-0 
                                    ${styles.inputs}
                                    ${(enableErrors.email && cleanForm === false) ? (error.user_email ? 'is-invalid' : 'is-valid') : 'form-control'}`}
                                    placeholder='Email *'
                                    name='user_email'
                                ...
                                />
                                {
                                    error.user_email && <p className="invalid-feedback mb-0">{error.user_email}</p>
                                }
                            </div>
                            <div className='mb-3'>
                                <input
                                    ...
                                    ms-0 
                                    ${styles.inputs}
                                    ${(enableErrors.affair && cleanForm === false) ? (error.user_affair ? 'is-invalid' : 'is-valid') : 'form-control'}`}
                                    placeholder='Asunto *'
                                    name='user_affair'
                                ...
                                />
                                {
                                    error.user_affair && <p className="invalid-feedback mb-0">{error.user_affair}</p>
                                }
                            </div>
                            ...
                            <div className={`${styles.container_TextArea}`} style={{ marginTop: "10px" }}>
                                <textarea
                                    ...
                                    form-control 
                                    ${styles.inputs}
                                    ${(enableErrors.message && cleanForm === false) ? (error.user_message ? 'is-invalid' : 'is-valid') : 'form-control'}`}
                                    autoComplete='off'
                                    placeholder='Mensaje *'
                            ...
                            <div
                                ...
                                ${((submittedForm === true) && (submittedFormTrue === false)) ? styles.open : styles.container_AlertSuccess}`}
                                role="alert">
                                <svg
                                    xmlns="http://www.w3.org/2000/svg"
                                    width="24"
                                ...
                                <button type="button" className="btn-close" onClick={(e) => closeSuccessAlert(e)} aria-label="Close"></button>
                            </div>
